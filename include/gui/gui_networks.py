...
from .gui_toplevels import creationToplevelGUI, trainToplevelGUI,inferenceToplevelGUI
...
class controlNetworksFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        network_edition_l.place(x=1,y=40)
        self.network_edition = ttk.Combobox(net_canvas,state="readonly")
        self.network_edition['values'] = (netco.SOC,netco.WENG)
        self.network_edition.current(0)
        self.network_edition.place(x=111,y=40)

        def callback(network_frame):
            if network_frame.network_edition.get() == netco.WENG:
                if not os.path.exists(os.getcwd()+"/models/"+netco.WENG):
                    network_frame.model['values'] = list([""])
                else:
                    network_frame.model['values'] = list(os.listdir(os.getcwd()+"/models/"+netco.WENG))
            elif network_frame.network_edition.get() == netco.SOC:
                if not os.path.exists(os.getcwd()+"/models/"+netco.SOC):
                    network_frame.model['values'] = list([""])
                else:
                    network_frame.model['values'] = list(os.listdir(os.getcwd()+"/models/"+netco.SOC))

        model_l = tk.Label(net_canvas,text="Model:",bg=net_canvas['bg'],fg = parent.theme.selected_fg)
        model_l.place(x=1,y=60)
        self.model = ttk.Combobox(net_canvas,state="readonly")
        if self.network_edition.get() == netco.WENG:
            if not os.path.exists(os.getcwd()+"/models/"+netco.WENG):
                self.model['values'] = list([""])
            else:
                self.model['values'] = list(os.listdir(os.getcwd()+"/models/"+netco.WENG))
        elif self.network_edition.get() == netco.SOC:
            if not os.path.exists(os.getcwd()+"/models/"+netco.SOC):
                self.model['values'] = list([""])
            else:
                self.model['values'] = list(os.listdir(os.getcwd()+"/models/"+netco.SOC))
