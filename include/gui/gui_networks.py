...
from .gui_toplevels import (
    creationToplevelClassificationGUI,
    trainToplevelClassificationGUI,
)
from .gui_toplevels import (
    creationToplevelControlGUI,
    trainToplevelControlGUI,
    inferenceToplevelControlGUI,
)
...
class classficiationNetworksFrame(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        net_canvas = tk.Canvas(
            ...
            width=self.grandparent.width,
            height=self.grandparent.height,
        )
        ...
        def callback(network_frame):
            if network_frame.network_edition.get() == netco.TREND:
                if not os.path.exists(
                    os.path.join(os.getcwd(), "models/" + netco.TREND)
                ):
                    network_frame.model["values"] = list([""])
                else:
                    network_frame.model["values"] = list(
                        os.listdir(os.getcwd() + "/models/" + netco.TREND)
                    )
            elif network_frame.network_edition.get() == netco.CYCLES:
                if not os.path.exists(
                    os.path.join(os.getcwd(), "models/" + netco.CYCLES)
                ):
                    network_frame.model["values"] = list([""])
                else:
                    network_frame.model["values"] = list(
                        os.listdir(os.getcwd() + "/models/" + netco.CYCLES)
                    )

        model_l = tk.Label(
            net_canvas, text="Model:", bg=net_canvas["bg"], fg=parent.theme.selected_fg
        )
        ...
        create_button = tk.Button(
            net_canvas,
            text="Create",
            command=lambda: self.gen_create_box(),
            height=2,
            width=20,
        )
        ...
        train_button = tk.Button(
            net_canvas,
            text="Train",
            command=lambda: self.gen_train_box(),
            height=2,
            width=20,
        )
        ...
        inference_button = tk.Button(
            net_canvas,
            text="Infer",
            command=lambda: self.gen_inference_box(),
            height=2,
            width=20,
        )
...
class controlNetworksFrame(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.controller = ttk.Combobox(
            net_canvas, state="readonly", postcommand=self.callback_control
        )
