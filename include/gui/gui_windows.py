...
class windowsFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        windows_set_canvas = tk.Canvas(self,bg=parent.theme.selected_bg,width = grandparent.width,height=grandparent.height)
        windows_set_canvas.grid(row=0,column=0)
        label = tk.Label(windows_set_canvas,text="Speed Trend Window Settings",bg=windows_set_canvas['bg'],fg = parent.theme.selected_fg)
        label.place(x=1,y=10)

        trend_window_size_l = tk.Label(windows_set_canvas,text="Window Size:",bg=windows_set_canvas['bg'],fg = parent.theme.selected_fg)
        trend_window_size_l.place(x=1,y=40)
        trend_window_size = ttk.Combobox(windows_set_canvas,state="readonly")
        trend_window_size['values'] = (3,5,9,15,30,60)
        trend_window_size.current(0)
        trend_window_size.place(x=111,y=40)

        trend_window_step_l = tk.Label(windows_set_canvas,text="Window Step:",bg=windows_set_canvas['bg'],fg = parent.theme.selected_fg)
        trend_window_step_l.place(x=1,y=60)
        trend_window_step = ttk.Combobox(windows_set_canvas,state="readonly")
        trend_window_step['values'] = (1,3,5,10,15,30)
        ...
        label = tk.Label(windows_set_canvas,text="Engine Cycles Window Settings",bg=windows_set_canvas['bg'],fg=parent.theme.selected_fg)
        ...
        cycles_window_step_l = tk.Label(windows_set_canvas,text="Window Step:",bg=windows_set_canvas['bg'],fg=parent.theme.selected_fg)
