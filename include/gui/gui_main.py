...
def trend_windows_execute(window_size, window_step):
    ...
    if window_step <= window_size:
        model_path = os.path.join(
            ...
            netco.CLASSIFIERS,
            netco.TREND,
            "model_" + str(window_size) + "_" + str(window_step),
        )
        if not os.path.exists(model_path):
            ...
            trendProcess(
                data_path, model_path, features_list, measurements, window_settings
            )
        else:
            print("~$> The Model already exists.")
            fit_df = pd.read_csv(
                os.path.join(model_path, "train.csv"),
                usecols=features_list,
                engine="python",
            )
            fit_df.plot()
            plt.show()
        ...
        print(
            "The Window size cannot be smaller than the Window step. Correct your values."
        )
