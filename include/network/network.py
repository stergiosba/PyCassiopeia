...
class Network:
    ...
    def train(
        self, data, epochs, learning_rate, minibatch_size, shuffle, test_size, outputs
    ):
        ...
        with self.graph.as_default():
            ...
            feature_configs = {
                "x": tf.io.FixedLenFeature(shape=[n_y], dtype=tf.float32),
            }
        ...
        with tf.compat.v1.Session(graph=self.graph) as sess:
            ...
            table = tf.contrib.lookup.index_to_string_table_from_tensor(
                tf.constant([str(i) for i in range(n_y)])
            )
            ...
            train_writer = tf.compat.v1.summary.FileWriter(
                self.version_path + "/train", sess.graph
            )
            ...
            with tqdm(total=epochs, desc="Cost:  ", unit="epo") as pbar:
                for epoch in range(epochs):
                    ...
                    seed = seed + 1
                    minibatches = nets.random_mini_batches(
                        X_train, Y_train, minibatch_size, seed
                    )
                    with tqdm(
                        total=num_minibatches, desc="Progress:  ", unit="mini"
                    ) as minibar:
                        for minibatch in minibatches:
                            ...
                            minibar.update(n=1)
                    self.costs_test.append(
                        round(sess.run(cost, feed_dict={X: X_test, Y: Y_test}), 4)
                    )
                    self.costs_train.append(round(epoch_cost, 4))

                    if self.edition == netco.TREND or self.edition == netco.CYCLES:
                        correct_prediction = tf.equal(
                            output_function(final), output_function(Y)
                        )
                        ...
                        predictions_train = sess.run(
                            output_function(final), feed_dict={X: X_train}
                        )
                        predictions_test = sess.run(
                            output_function(final), feed_dict={X: X_test}
                        )
            ...
            classification_outputs_classes = (
                tf.compat.v1.saved_model.utils.build_tensor_info(prediction_classes)
            )
            classification_outputs_scores = (
                tf.compat.v1.saved_model.utils.build_tensor_info(values)
            )
            ...
            builder.add_meta_graph_and_variables(
                ...
                signature_def_map={
                    ...
                    tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY: classification_signature,
                },
            ...
            self.predictions_train = sess.run(
                output_function(final), feed_dict={X: X_train}
            ).T
            self.predictions_test = sess.run(
                output_function(final), feed_dict={X: X_test}
            ).T
            self.predictions = np.concatenate(
                (self.predictions_train, self.predictions_test), axis=0
            )
            ...
            self.ordered_predictions = sess.run(
                output_function(final), feed_dict={X: X_ordered}
            ).T
    ...
    def init_layers(self):
        ...
        for layer_counter, layer in enumerate(self.structure, start=0):
            ...
            with tf.compat.v1.variable_scope("Weights"):
                self.layers[name_W] = tf.compat.v1.get_variable(
                    name_W,
                    [int(layer[0]), int(layer[1])],
                    initializer=tf.contrib.layers.xavier_initializer(seed=1),
                )
            with tf.compat.v1.variable_scope("Biases"):
                self.layers[name_b] = tf.compat.v1.get_variable(
                    name_b, [int(layer[0]), 1], initializer=tf.zeros_initializer()
                )
        ...
        for layer_counter, layer in enumerate(self.structure, start=0):
            ...
            if activation_function == netco.LINEAR:
                ...
                A.append(
                    tf.add(
                        tf.matmul(
                            self.layers["W" + str(layer_counter)], Z[layer_counter]
                        ),
                        self.layers["b" + str(layer_counter)],
                    )
                )
                ...
                raise NameError(
                    "There is an error with the activation functions in network_structure.json. Check the file and retry."
                )
        # [Return last layer]
        return Z[-1]
        ...
        if self.function == netco.TRAINING:
            ...
            for column in data.columns:
                if column == "D_MAX" or column == "D_AVE":
                    final_df[column] = data[column].apply(
                        lambda x: 1
                        - (x - normalizers.loc[column].min())
                        / (
                            normalizers.loc[column].max()
                            - normalizers.loc[column].min()
                        )
                    )
                else:
                    final_df[column] = data[column].apply(
                        lambda x: (x - normalizers.loc[column].min())
                        / (
                            normalizers.loc[column].max()
                            - normalizers.loc[column].min()
                        )
...
class NNClassifier(Network):
    ...
    def train(
        ...
        super().train(
            data, epochs, learning_rate, minibatch_size, shuffle, test_size, outputs
        )
        self.plot_accuracy()
        self.plot_costs()
    ...
    def plot_costs(self):
        ...
        ax.set(
            xlabel="Epochs",
            ylabel="Softmax Cross Entropy",
            title="Training and Testing Errors for " + self.edition,
        )
        ax.grid()
        plt.legend()
    ...
    def plot_accuracy(self):
        ...
        ax.set(
            xlabel="Epochs",
            ylabel="Accuracy",
            title="Training and Testing Accuracy for " + self.edition,
        )
        ax.grid()
        plt.legend()
    ...
    def inference(self, data, window_settings):
        ...
        with self.graph.as_default():
            saver = tf.compat.v1.train.import_meta_graph(
                self.version_path + "/model.ckpt.meta"
            )
        self.layers = {}
        for op in self.graph.get_operations():
            for layer_counter, layer in enumerate(self.structure, start=0):
                ...
                if re.search(pattern_W, str(op.name)) is not None:
                    self.layers["W" + str(layer_counter)] = tf.cast(
                        self.graph.get_tensor_by_name(str(op.name) + str(":0")),
                        tf.float64,
                        name="W" + str(layer_counter),
                    )

                if re.search(pattern_b, str(op.name)) is not None:
                    self.layers["b" + str(layer_counter)] = tf.cast(
                        self.graph.get_tensor_by_name(str(op.name) + str(":0")),
                        tf.float64,
                        name="b" + str(layer_counter),
                    )
        # Start reading datas
        ...
        for i, x in enumerate(x_lines):
            if i == 0:
                ...
                if i % 2 == 0:
                    ax.text(
                        int(xs.mean()), 0.92, CLASSES[label], color="blue", fontsize=20
                    )
                else:
                    ax.text(
                        int(xs.mean()), 0.86, CLASSES[label], color="blue", fontsize=20
                    )
        ...
        ax.set(
            xlabel=r"$\mathbf{Time}$ (sec)",
            ylabel=r"$\mathbf{\beta}$ / $\mathbf{\beta_{max}}$",
        ...
        fig.savefig(
            os.path.join(
                self.version_path, data.split(".")[0] + "_cycle_inference.png"
            ),
            dpi=800,
        )

        pd.DataFrame(predictions).to_csv(
            os.path.join(self.version_path, data.split(".")[0] + "_cycle_inference.csv")
        )
        full_df.to_csv(
            os.path.join(self.version_path, data.split(".")[0] + "_cycle_data.csv")
        )
...
class NNRegressor(Network):
    ...
    def plot_trained(self, cycle):
        ...
        ax.set(
            xlabel="Time [Secs]",
            ylabel="Torque [Nm]",
            title=self.edition + " Torque (Cycle " + cycle + ")",
        )
        ax.grid()
        plt.legend()
        ...
        frame.to_csv(
            os.path.join(
                self.build_path, "results_" + self.edition + "_" + cycle + ".csv"
            ),
            index=False,
        )
    ...
    def plot_predictions(self, cycle):
        ...
        ax.set(
            xlabel="True Values [Torque]",
            ylabel="Predictions [Torque]",
            title=self.edition + " Predictions (Cycle " + cycle + ")",
        )
        ax.axis("equal")
        ax.axis("square")
        ax.grid()
    ...
    def plot_costs(self, cycle):
        ...
        ax.set(
            xlabel="Epochs",
            ylabel="Mean Absolute Error (Torque)",
            title=self.edition + " Training and Testing Errors (Cycle " + cycle + ")",
        )
        ax.grid()
        plt.legend()
